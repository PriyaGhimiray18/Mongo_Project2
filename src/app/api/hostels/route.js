...
export async function PUT(request) {
  ...
  try {
    ...
    return Response.json(updatedHostel, { status: 200 });
  } catch (error) {
    console.error('❌ PUT /api/hostels error:', error);
    return Response.json({ error: 'Failed to update hostel' }, { status: 500 });
  }
}
